...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [updated] = await db.update(speakers).set(updateData).where(eq(speakers.id, parseInt(id))).returning()
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params
  await db.update(speakers).set({ is_active: false, updated_at: new Date() }).where(eq(speakers.id, parseInt(id)))
