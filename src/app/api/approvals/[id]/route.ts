...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [approval] = await db.select().from(approvals).where(eq(approvals.id, numId))
    ...
    const steps = await db
      ...
      .where(eq(approval_steps.approval_id, numId))
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Update the step and parent approval atomically
    const newStatus = action === 'approve' ? 'approved' : 'rejected'

    await db.transaction(async (tx) => {
      await tx.update(approval_steps).set({
        status: newStatus,
    ...
    logActivity({ userId: session.id, action: 'updated', resource: 'approval', resourceId: numId, details: JSON.stringify({ action: newStatus }) }).catch(() => {})
