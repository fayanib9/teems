...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!metric_name?.trim()) return NextResponse.json({ error: 'metric_name is required' }, { status: 400 })
    if (!target_value) return NextResponse.json({ error: 'target_value is required' }, { status: 400 })
