...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [updated] = await db.update(vendors).set(updateData).where(eq(vendors.id, parseInt(id))).returning()
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params
  await db.update(vendors).set({ is_active: false, updated_at: new Date() }).where(eq(vendors.id, parseInt(id)))
