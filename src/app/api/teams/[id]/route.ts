...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const members = await db
      ...
      .where(eq(team_members.team_id, numId))
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [updated] = await db.update(teams).set(updateData).where(eq(teams.id, numId)).returning()
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [updated] = await db.update(teams).set({ is_active: false }).where(eq(teams.id, numId)).returning({ id: teams.id })
    if (!updated) return NextResponse.json({ error: 'Team not found' }, { status: 404 })
