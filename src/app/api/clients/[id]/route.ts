...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [updated] = await db.update(clients).set(updateData).where(eq(clients.id, parseInt(id))).returning()
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params
  // Soft delete
  const [updated] = await db.update(clients).set({ is_active: false, updated_at: new Date() }).where(eq(clients.id, parseInt(id))).returning({ id: clients.id })
  if (!updated) return NextResponse.json({ error: 'Client not found' }, { status: 404 })
